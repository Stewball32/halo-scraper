...
// findPID scans /proc for the AppRun process that references the given QMP socket.
func findPID(qmpSock string) (int, error) {
	sockToken := filepath.Base(qmpSock) // e.g. "xemu-host-02.sock"
	...
	for _, entry := range entries {
		...
		if strings.HasSuffix(exe, "AppRun") && strings.Contains(cmdline, sockToken) {
	...
	return 0, fmt.Errorf("no AppRun process found matching %s", sockToken)
}
